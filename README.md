...
> **定位**：统一用户入口，聚合需求、商城、工作台、任务、审批、结果与个人事项，只流转不执行
> **不负责**：任何领域对象的权威与确定性执行（归各业务软件）
> **完成度（2026-09-01）**：主体完成——IAM/ABAC/多租户/i18n/SLA/数据保留已产品化，114 前端测试+52 后端测试全绿
> **真实/Stub 边界**：前端含显式开启的本地 mock 模式（VITE_API_MODE=mock，默认走真实 API）；任务与审批依赖各业务软件真实接口
